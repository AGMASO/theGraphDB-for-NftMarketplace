...
export class ItemListing extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save ItemListing entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type ItemListing must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("ItemListing", id.toString(), this);
    }
  }

  static load(id: string): ItemListing | null {
    return changetype<ItemListing | null>(store.get("ItemListing", id));
  }
